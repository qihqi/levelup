...
更新后需重启服务才能加载新代码；房间自动存入 SQLite，重启后可恢复。

## 开始玩

1. 输入昵称创建房间。也可填写朋友的六位房间号加入。
2. 选择「记牌策略 AI」（默认）、「基础 AI」、「XGBoost 出牌 AI」、「搜索 AI（最多 30 秒）」或「OpenAI Agent AI」（需配置 API）。房主复制邀请链接，朋友须在开局前入座；点击空位可换座，对面是队友。
3. 房主点击「开始游戏」，空位自动由 AI 补齐。一个人可以立即开局。
4. 开局进入摸牌：全桌每 0.5 秒按座位顺序摸一张，摸到级牌或王对即可选牌点击「亮主 / 反主」，无需等自己的回合。摸完后最多留 60 秒最后亮主，可点「不亮」；无法亮主 / 反主或当前主由自己亮出的玩家自动确认，全员确认后提前定主，再由庄家扣底、轮流出牌。「提示」只选择建议牌，不自动提交；展开「提示依据与备选出牌」可以查看前三名动作和每条规则的加减分。
5. 浏览器保存随机访客身份与昵称。大厅的「我的未完成牌桌」可回到原座位；任何真人离线都会暂停牌局，所有真人重新入座后继续。可主动开启或取消托管。
6. 一局结束后由创建牌桌的房主开始下一局；房主身份不会因离线转移。房主可通过「管理玩家」移除其他玩家，由 AI 接管其手牌。坐庄打过 A 后可开始新比赛。

房主可在「开始游戏」上方选择出牌时限：15、30、60、120 秒或不限时；默认 60 秒。扣底为出牌时限的 1.5 倍并向上取整，默认 90 秒。也可在局间通过「操作时限」修改，设置对全桌生效并保留到下一局和新比赛。超时只代行本次操作，不会永久开启托管；不限时会关闭真人出牌与扣底的倒计时，AI 与主动托管仍正常出牌。摸牌通常约 50 秒，亮主不会暂停或重置摸牌计时。缺少任何真人时整桌暂停，摸牌、最后亮主和操作倒计时都冻结；归队后保留剩余时间继续。

## 存档与恢复

首次创建或加入房间时，服务器生成随机访客身份，浏览器将它与昵称保存在 localStorage。大厅自动显示该身份的未完成牌桌，包括等待开局、进行中和局间休息的桌子；整场比赛结束后不再列出。两位真人同桌时，两人都需要回来才能继续；房主也可移除缺席者，让 AI 补位。被移除者不能中途返回该座位。

每次有效操作和自动摸牌都会在广播前提交 SQLite 存档，包含四家手牌、底牌、出牌历史、比分、级别、AI 设置、座位和剩余计时。服务重启后，房间按需加载，等全部真人归队再继续。六小时无人在线只会回收内存，数据库中的存档保留。

默认数据库为 `~/.local/share/levelup/state.sqlite3`；设置了 `XDG_DATA_HOME` 时使用其下的 `levelup/state.sqlite3`。可用 `LEVELUP_DB_PATH=/path/to/state.sqlite3` 指定固定位置。数据库路径不依赖启动目录，升级程序时请保留数据库。systemd 的可写目录、备份和限制见 [持久化部署说明](docs/PERSISTENCE.md)。

访客身份属于当前浏览器和网站地址（含协议、主机名、端口）。清除网站数据、使用无痕窗口或换设备后不能自动找回；这不是跨设备账户系统。更新到这个版本前仅存在旧服务内存中的房间无法自动迁移。
...
- **边摸边亮**：从庄家位置开始，全桌每 0.5 秒给下一位发一张，直到每人 25 张；真人与 AI 随时可用已摸到的牌亮主或反主。最后一张发出后留 60 秒；庄家可点「不改主」，其他玩家可点「不亮」，没有合法亮主 / 反主动作的玩家，以及当前亮主者，自动确认，无需点击。四人均确认则提前定主。底牌在定主前不会发给庄家；仍可反主的玩家尚未确认时，继续等待至倒计时结束。新亮主/反主会重置 60 秒及全员确认，AI 也会自动选择亮主或不亮。单级牌 < 同花色对级牌 < 对小王 < 对大王；王对亮无主。可用同花色对子加固自己，不能反自己。同强度不能反主，以服务器先收到的有效声明为准。
...
策略只接收自己的手牌、公开牌张/张数与庄家本人知道的底牌，不接触其他人的手牌。「XGBoost 出牌 AI」使用 2,000 次出牌决策及全信息教师标签训练的小型排序模型；亮主与扣底沿用记牌策略。教师仅在小残局穷举求解，其他标签由全信息续局估值产生，不保证绝对最优。模型随 wheel 分发，实际运行不需要安装 XGBoost。训练方法、局限与复现命令见 [出牌模型说明](docs/PLAY_MODEL.md)；启发式来源与权重见 [AI 策略说明](docs/AI_STRATEGIES.md)。

「搜索 AI」用记牌与 XGBoost 推荐候选，在符合公开信息的模拟手牌中比较续局结果。每步最多搜索 28 秒，预留提交时间；较短房间时限会相应缩短搜索。搜索在独立进程中运行，牌桌会显示思考状态；等待搜索提示时仍可手动出牌。亮主、扣底与超时兜底使用记牌策略。原理、日志和对比命令见 [搜索 AI 说明](docs/SEARCH_AI.md)。

「OpenAI Agent AI」通过官方 OpenAI Python SDK，让大模型读取本桌简明规则、自己的手牌、全部公开出牌历史和合法候选，选择本次出牌并给出简短理由。默认使用 `gpt-5-mini`，可通过 `LEVELUP_OPENAI_MODEL` 修改；安装 `.[agent]` 并在服务器环境中设置 `OPENAI_API_KEY` 后使用。每次最多等待 28 秒，出错时使用记牌策略；亮主与扣底仍由记牌策略完成。支持离线模拟和训练日志；模型推理需要联网且产生 API 费用。安装、提示词、日志与配置见 [OpenAI Agent 说明](docs/OPENAI_AGENT.md)。
...
levelup/
  ...
  storage.py         SQLite 访客身份、房间成员与版本化 JSON 存档
  simulation.py      离线模拟 API、CLI、JSONL 结果与命令日志
  training.py        自博弈训练数据快照、元数据与样本读取
  god_teacher.py     全信息续局教师与小残局穷举
  train_play.py      出牌标签生成、XGBoost 训练与评估
  models/           随包分发的出牌模型与评估元数据
...
服务器为每个玩家生成独立视图，只传自己已经摸到的手牌、公开牌张及其他人的张数；底牌只在结算时公开。每个房间使用异步锁串行处理操作，客户端提交状态版本以拒绝过期操作；摸牌期间亮主和提示额外带 `deal_id`，同次摸牌允许较旧版本，仍核验当前手牌和反主强度。访客身份保存在 localStorage，通过首条 WebSocket 消息和 HTTP Authorization 头传递，不放在邀请链接内；兼容旧版 sessionStorage 中的单桌恢复令牌。数据库只存全局身份的哈希，通信日志会脱敏身份和令牌。

这是**单进程、SQLite 存档版本**，不包含注册账户、观战或中途新玩家入座。运行一个 Uvicorn worker；活动房间与 WebSocket 仍在内存中，多个 worker 不能协同操作同一房间。线上扩容需先引入共享房间协调与消息分发。默认最多同时加载 200 个房间，六小时无人在线后卸载内存，恢复时从数据库重新加载。
...
`--trace` 不包含候选动作与其他玩家的手牌；采集训练数据时使用下方的 `--training`。
日志可能包含手牌，且不会自动轮转；大量模拟默认只记录结果，按需开启 trace 并管理文件大小。

### 自博弈训练数据

```bash
~/.venv/bin/python -m levelup.simulation --games 100 --seed 0 \
  --training --exploration 0.10 --output logs/training/selfplay-000.jsonl.gz
```

`--training` 自动包含命令追踪，并记录每次决策的玩家视角、四家完整手牌、底牌、剩余牌堆、所有实际参与排名的候选动作及分数、所选动作与选择概率。出牌命令另外记录实际出的牌（甩牌失败时可能与请求不同），以及完成的墩和得分。每个决策用 `decision_id` 关联局末胜负奖励；`levelup.training.iter_decisions(path)` 可流式读取合并后的样本。

`--exploration 0.10` 表示 10% 的决策从候选中均匀抽样，其余选择排名第一的动作；默认 0 保持原 AI 行为。探索 RNG 与洗牌 RNG 分开，可用 `--policy-seed` 指定探索种子。`.gz` 输出自动压缩。多进程采集时每个进程使用独立文件和不同种子范围。候选集来自现有有界生成器，未选动作的分数是策略评分，不是模拟得到的价值标签。字段与读取示例见 [训练数据说明](docs/TRAINING_DATA.md)。
...
发布前请阅读 [安全与部署说明](SECURITY.md) 和 [本次安全审计](docs/SECURITY_AUDIT_2026-09-12.md)。
不要提交 API 密钥或私有日志。公网服务需要 HTTPS、访问控制与资源限制；匿名访客可消耗服务器的 AI API 额度。
可用 `LEVELUP_WEB_AI_STRATEGIES=rule_based` 在服务器端只开放记牌策略；重新启动后生效。
...
日志从加载此版本的服务启动后开始，无法补回之前未记录的操作。现有服务需要重启才能启用；已写入 SQLite 的房间会保留。

## WebSocket 协议

`GET /api/ai-strategies` 获取注册策略及默认策略。

`POST /api/identity` 请求 `{"name":"昵称","identity":"已有身份，可省略"}`，创建或更新访客档案，返回 `identity` 和 `name`。首次创建或加入房间也会自动创建身份。

`GET /api/me/rooms` 使用 `Authorization: Bearer <identity>`，返回昵称及仅属于此身份的未完成牌桌摘要；不含任何手牌、底牌或令牌。

`POST /api/rooms` 请求 `{"name":"昵称","ai_strategy":"rule_based","identity":"已有身份，可省略"}`（策略可省略），返回 `room`、`token`、`identity` 和 `name`。

连接 `/ws/{room}`，首条消息发送 `{"name":"昵称","identity":"访客身份","token":"旧版恢复令牌或空字符串"}`。身份即可恢复已有座位，`token` 可省略；新访客可不带身份。

服务器发送 `welcome`（座位、令牌、身份与昵称）和 `state`（当前玩家的完整视图）。后续操作带上最近一次 `state.version`：
...
{"action":"kick", "seat":1, "version":1002}
```
...
`state.paused` 表示等待真人归队，`waiting_for` 为缺席座位数组。暂停时拒绝开局、出牌、亮主等推进牌局的操作。仅房主可 `kick` 其他真人，由 AI 接管该座位；被移除者的连接以 `4005` 关闭，其身份的未完成牌桌列表会移除本桌。

摸牌阶段为 `phase: "dealing"`；`dealt` / `deal_total` 为全桌进度，`turn` 为下一张牌的接收座位，`last_draw_seat` 为刚摸牌的座位。`deal_closing` 和 `deal_seconds_left` 表示最后亮主窗口，`draw_interval` 表示发牌间隔。`deal_id` 在新局和无人亮主重发时更新。亮主不受 `turn` 限制，不操作即不亮主；摸牌中提示的 `pass` 表示等待；摸完后可发送 `pass` 确认不亮。请求须带当前 `deal_id`、`version` 和 `bid_revision`（亮主次数），同一亮主状态允许并发确认；新亮主清空 `bid_passed`，重置 60 秒窗口。`state.bid_passed` 为已确认（含自动确认）的座位数组。摸完后服务器自动确认无法亮主 / 反主的玩家及当前亮主者；每次反主后重新判断。`state.bid_skip_reason` 仅描述当前玩家：`own_bid` 表示当前主由自己亮出，`no_legal_bid` 表示无合法亮主 / 反主动作，`null` 表示不自动跳过。
